...
public class GameOver : MonoBehaviour
{
    ...
    void OnEnable ()
    {
        ...
        int totalLevelsDone = GameManager.instance.levelsCompleted;
        roundsText.text = $"You completed {totalLevelsDone} levels!";

        if (GameManager.instance.RemainingTime <= 0 && totalLevelsDone >= GameManager.instance.minLevelsForTimerEndCondition)
            {
                 roundsText.text += "\nTime's up!";
             }
